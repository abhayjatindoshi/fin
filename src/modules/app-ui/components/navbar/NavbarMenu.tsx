...
const NavbarMenu: React.FC<NavbarMenuProps> = ({ className, variant = 'default' }: NavbarMenuProps) => {
    ...
    const selectedTab = menu.find(item => location.pathname.startsWith('/' + currentTenant?.id + item.url));
                ...
                {menu.map(item => (
                    <TabsTrigger value={item.key} key={item.key} onClick={() => switchTab(item)}
                        className="px-3 py-1 rounded-3xl text-base border-0 cursor-pointer relative
                                after:absolute after:-top-1 after:h-[calc(var(--spacing)*12-2px)] after:bg-[radial-gradient(ellipse_at_bottom,var(--accent),rgba(255,255,255,0)_50%)] after:w-[calc(100%+calc(var(--spacing)*3)+13px)] first:after:rounded-l-3xl last:after:rounded-r-3xl after:opacity-0 data-[state=active]:after:opacity-50 after:transition-opacity after:duration-500 hover:after:opacity-25
