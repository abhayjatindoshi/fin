...
export class MetadataManager<U extends EntityUtil<SchemaMap>> {
    ...
    async saveMetadata(persistence: IPersistence, metadata: Metadata): Promise<void> {
        const entityKeyData = this.toEntityKeyData(metadata);
        await persistence.storeData(this.metadataKey, entityKeyData);
        switch (this.which(persistence)) {
            case "store":
    ...
    private getPersistence(type: PersistenceType): IPersistence {
        switch (type) {
            case "store":
                return this.store;
            case "local":
    ...
    private async getMetadataOrDefault(type: PersistenceType): Promise<Metadata> {
        ...
        const data = await persistence.loadData(this.metadataKey);
    ...
    private which(persistence: IPersistence): PersistenceType {
